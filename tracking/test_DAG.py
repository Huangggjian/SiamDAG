...
def parse_args():
    parser = argparse.ArgumentParser(description='Test DAG')
    parser.add_argument('--arch', dest='arch', default='DAGTRT', help='backbone architecture')
...
def track(siam_tracker, online_tracker, siam_net, video, args):
    ...
    with open(result_path, "w") as fin:
        ...
        elif 'OTB' in args.dataset or 'LASOT' in args.dataset or 'LaSOT' in args.dataset:
            for x in regions:
                p_bbox = x.copy()
                fin.write(
                    ','.join([str(i + 1) if idx == 0 or idx == 1 else str(i) for idx, i in enumerate(p_bbox)]) + '\n')
        elif 'VISDRONE' in args.dataset or 'GOT10K' in args.dataset or 'UAV' in args.dataset:
            for x in regions:
                p_bbox = x.copy()
...
def main():
    ...
    info = edict()
    info.arch = args.arch
    info.dataset = args.dataset             # vot2019
    info.TRT = 'TRT' in args.arch
    info.epoch_test = args.epoch_test       # bool
    ...
    if args.online:
        ...
        siam_info.align = True if 'VOT' in args.dataset and args.align == 'True' else False     # false
    ...
    # run this
        ...
        for video in video_keys:
            # the track func is in this file
