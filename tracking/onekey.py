...
def main():
    ...
    if trainINFO['ISTRUE']:
        print('==> train phase')
        print('python ./tracking/train_DAG.py --cfg {0} --gpus {1} --workers {2} 2>&1 | tee logs/DAG_train.log'
                  .format(args.cfg, info['GPUS'], info['WORKERS']))

        if not exists('logs'):
            os.makedirs('logs')

        os.system('python ./tracking/train_DAG.py --cfg {0} --gpus {1} --workers {2} 2>&1 | tee logs/DAG_train.log'
                  .format(args.cfg, info['GPUS'], info['WORKERS']))
    ...
    if testINFO['ISTRUE']:
        ...
        if 'VOT' in testINFO['DATA']:
            ...
            os.system('python lib/eval_toolkit/bin/eval.py --dataset_dir dataset --dataset {0} \
            --tracker_result_dir result/{0} --trackers {1} 2>&1 | tee logs/DAG_eval_epochs.log'.format(testINFO['DATA'], trackers))
        else:
