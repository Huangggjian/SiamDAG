...
def load_dataset(dataset):
    ...
    if 'OTB' in dataset:
    ...
    elif 'UAV123' in dataset:    #  UAV123
        base_path = join(realpath(dirname(__file__)), '../../dataset', dataset)
        seq_path = join(base_path, 'data')
        gt_paths = join(base_path, 'anno', 'UAV123')
        ...
        for video in videos:
            ...
            image_path = join(video_path, '*.jpg')
            image_files = sorted(glob.glob(image_path))
            gt_path = join(gt_paths, str(video) + '.txt')
            gt = np.loadtxt(gt_path, delimiter=',')
...
def create_logger(cfg, modelFlag='DAG', phase='train'):
...
#    old_filename = filename.split('.')[0] + '_old.pth'
#    torch.save(states, os.path.join(output_dir, old_filename),_use_new_zipfile_serialization=False)
...
def _build_warm_up_scheduler(optimizer, cfg, epochs=50, last_epoch=-1, modelFLAG='DAG'):
    #cfg = cfg[modelFLAG]
...
def build_lr_scheduler(optimizer, cfg, epochs=50, last_epoch=-1, modelFLAG='DAG'):
    cfg = cfg[modelFLAG]
