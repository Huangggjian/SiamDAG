...
class OTBDataset(Dataset):
    ...
    """
    def __init__(self, name, dataset_root, load_img=False):   # OTB100 ,dataset
        super(OTBDataset, self).__init__(name, dataset_root)
        with open(os.path.join(dataset_root, name+'.json'), 'r') as f:  # open OTB100.json in ./dataset
        ...
        ###########################################################
        for video in pbar:    # video is name like Basketball , human4-2
            pbar.set_postfix_str(video)
            self.videos[video] = OTBVideo(video,
                                          dataset_root,
                                          meta_data[video]['video_dir'],    # Human4-2
                                          meta_data[video]['init_rect'],    # [99, 237, 27, 82]
                                          meta_data[video]['img_names'],    # Human4-2/img/0001.jpg....all imgs in a video
                                          meta_data[video]['gt_rect'],      # all gt box
                                          meta_data[video]['attr'],         # ["Illumination Variation", "Scale Variation", "Occlusion", "Deformation"]
                                          load_img)
