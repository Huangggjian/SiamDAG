...
from lib.eval_toolkit.pysot.datasets import OTBDataset, UAVDataset, LaSOTDataset, VOTDataset, NFSDataset, VOTLTDataset
from lib.eval_toolkit.pysot.evaluation import OPEBenchmark, AccuracyRobustnessBenchmark, EAOBenchmark, F1Benchmark
from lib.eval_toolkit.pysot.visualization import draw_success_precision, draw_eao, draw_f1
#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--show_video_level', dest='show_video_level', default=False)   # show every video detail info
    parser.add_argument('--num', type=int, help='number of processes to eval', default=16)
    ...
    if 'OTB' in args.dataset:
        dataset = OTBDataset(args.dataset, root)   # root: dataset
        ...
        if args.vis:
            for attr, videos in dataset.attr.items():
                draw_success_precision(success_ret,     # wrong here
                            name=dataset.name,
                            videos=videos,
                            attr=attr,
                            precision_ret=precision_ret)
    elif 'LaSOT' == args.dataset or 'LASOT' == args.dataset:
        dataset = LaSOTDataset(args.dataset, root)   # LaSOT , dataset
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        ...
        dataset = UAVDataset(args.dataset, root)    # UAV123   dataset
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        with Pool(processes=args.num) as pool:
