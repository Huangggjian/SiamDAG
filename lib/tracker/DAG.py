...
class DAG(object):
    ...
    def update(self, net, x_crops, target_pos, target_sz, window, scale_z, p):
        ...
        if self.align:
            ...
            cls_score, bbox_pred, quailty_pred = net.track(x_crops)

            quailty_pred = F.sigmoid(quailty_pred).squeeze().cpu().data.numpy()
            cls_score = F.sigmoid(cls_score).squeeze().cpu().data.numpy()

            cls_score = np.sqrt(cls_score * quailty_pred)
    ...
    def grids(self, p):
        ...
        x, y = np.meshgrid(np.arange(0, sz) - np.floor(float(sz_x)),  # -12 to 12
